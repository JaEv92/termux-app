...
final class TermuxInstaller {
    ...
    static void setupIfNeeded(final Activity activity, final Runnable whenDone) {
        ...
        // account has the expected file system paths.
        UserManager um = (UserManager) activity.getSystemService(Context.USER_SERVICE);
        boolean isPrimaryUser = um.getSerialNumberForUser(android.os.Process.myUserHandle()) == 0;
        if (!isPrimaryUser) {
            // Note, that if program is running under Knox container (Secure folder from Samsung)
            // paths could be same as running program as primary user (device owner). To verify
            // this, we should check uid of Termux folder (/data/data/com.termux).
            if (TermuxHelper.getFileUID("/data/data/com.termux") != TermuxHelper.getAppUID()) {
